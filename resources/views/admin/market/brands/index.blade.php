...
@section('content')
    <div class="col-12">
        ...
        <div class="card-text">
            <p>{!! trans('admin.market.brands.intro') !!}</p>
        </div>
        <div class="row">
            <div class="col-6">
                <div class="card shadow">
                    <div class="card-header">
                        <div class="d-flex justify-content-between">
                            <div class="h4">{{ trans('admin.market.brands.list') }}</div>
                        </div>
                    </div>
                                ...
                                @foreach ($brands as $b)
                                    <tr>
                                        ...
                                        <td>@include('admin._partials.dred', [
                                            ...
                                            'field' => 'slug',
                                            'model' => 'market.brand',
                                            'permission_group' => 'market-brands',
                                            'edit' => 1,
                                            'delete' => 1,
                                            'restore' => 1,
                                            'destroy' => 1,
            ...
            <div class="col-6">
                <div class="card shadow">
                    <h4 class="card-header">
                        {{ $brand ? trans('admin.market.brand.edit', ['name' => $brand->slug]) : trans('admin.market.brand.add') }}
                    </h4>
                    <div class="card-body">
                        <form
                            action="{{ $brand ? route('admin.market.brand.update', $brand->slug) : route('admin.market.brand.store') }}"
                            method="post">@csrf
                            <div class="input-group mb-2">
                                <span class="input-group-text">{!! getIcon('fas', 'tag') !!}</span>
                                <input type="text" class="form-control" name="name"
                                    value="{{ $brand ? $brand->name : old('name') }}" placeholder="{{ __('Name') }}"
                                ...
                                @role('admin')
                                    <div class="form-check form-switch">
                                        <input class="form-check-input" type="checkbox" user="switch" name="status"
                                            {{ ($brand && $brand->status ? ' checked' : !$brand) ? ' checked' : '' }}>
                    ...
                    <div class="card-footer">
                        <div class="d-flex justify-content-between">
                            <a href="{{ route('admin.market.brands') }}"
